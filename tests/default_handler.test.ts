import { ResultWrapper, App, Router, DefaultHandler, Path } from "../index";
...
describe('DefaultHandler tests', () => {
  ...
  test('TestRouter - /router/asd', async () => {
    await get_.then(e => e('/router/asd').expect(res => expect(res.text).toBe(default_text_router)));
  });

  test('TestRouter - /router', async () => {
    await get_.then(e => e('/router').expect(res => expect(res.text).toBe(default_text_router)));
  });
