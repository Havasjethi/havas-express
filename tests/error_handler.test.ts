...
describe('Test error handlers', () => {
  ...
  app.append(rotuer_1).append(rotuer_2);

  const get = get_request_creator(app);

  it('Test 1', async () =>
    await get('/').expect((res) => expect(res.text).equal(error_handler_message_main_app)));

  it('Test 2', async () =>
    await get('/router').expect((res) => expect(res.text).equal(error_handler_message_router)));

  it('Test 3', async () =>
    await get('/router-2').expect((res) => expect(res.text).equal(error_handler_message_main_app)));
});
